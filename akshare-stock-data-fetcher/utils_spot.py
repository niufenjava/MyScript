...
def getDayDate(allDate, dt): # pandas加速处理
    """
    :param allDate: 全部的股票数据
    :param dt: 获取某一天的日期格式为yyyy-mm-dd
    :return:  返回某一天的全部数据
    """
    df = pd.DataFrame(allDate)
    df["日期"] = df["时间"].str.split().str[0]
    grouped = df.groupby("日期")
    dayDate = grouped.get_group(dt).to_dict(orient='records')
    return dayDate
