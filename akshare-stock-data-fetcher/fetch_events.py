...
EVENTS_DIR = os.path.join(EVENTS_DIR, "events") if "EVENTS_DIR" in dir() else os.path.join(SCRIPT_DIR, "data", "events")
...
EVENT_TYPES = [EVENT_ZT, EVENT_DT, EVENT_ZB, EVENT_QSG, EVENT_LHB]  # 全部异动类型
...
def get_latest_trade_date(ref_date=None):
    """获取最近交易日，默认今天"""
...
def save_events(all_rows, mode="a"):
    """
    按异动类型分文件写入 CSV。
    - mode="w": 完全覆盖写入（用于强制刷新）
    - mode="a": 追加写入（只新增全新 key，保留已有）

    去重键：(股票代码, 日期, 异动类型)
    注意：强势股(回调) 视为独立类型，单独存文件
    """
    # 按类型分组
    by_type = {}
    for row in all_rows:
    ...
    for event_type, rows in by_type.items():
        fpath = get_event_csv_path(event_type)
        existing = {} if mode == "w" else _load_single_type(event_type)
        ...
        logger.info(f"[{event_type}] 写入 {len(existing)} 条（本次 {len(rows)} 条）")
...
def fetch_lhb_events(trade_dt):
    """
    拉取指定交易日的龙虎榜事件（用日期区间接口，逐日拉取）。
    龙虎榜接口参数：start_date, end_date（区间），返回多日数据
    """
    date_str = trade_dt.strftime("%Y%m%d")
    try:
...
def fetch_daily_events(trade_dt, existing_keys):
    ...
    try:
        ...
        lhb_rows = fetch_lhb_events(trade_dt)
